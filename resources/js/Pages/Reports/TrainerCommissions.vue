...
const summaryCards = computed(() => [
    ...
    { label: t('staff_reports.paid_commission'), value: formatAmount(props.summary.paid_commission_amount), icon: 'tabler-check', class: 'bg-label-info text-info' },
    ...
    { label: t('staff_reports.unearned_cancelled'), value: formatAmount(props.summary.cancelled_unearned_commission_amount), icon: 'tabler-ban', class: 'bg-label-danger text-danger' },
    { label: t('staff_reports.transferred_entry'), value: formatAmount(props.summary.transferred_in_amount), icon: 'tabler-arrow-down-left', class: 'bg-label-success text-success' },
    { label: t('staff_reports.transferred_exit'), value: formatAmount(props.summary.transferred_out_amount), icon: 'tabler-arrow-up-right', class: 'bg-label-secondary text-secondary' },
    { label: t('staff_reports.saved_commissions'), value: props.summary.kept_commissions_count ?? 0, icon: 'tabler-lock', class: 'bg-label-secondary text-secondary' },
])
...
const statusLabel = status => ({
    ...
    cancelled: t('staff_reports.generation_is_stopped'),
}[status] ?? status ?? '-')
...
<template>
    <Head :title="t('sidebar.trainer_commissions')" />

    <Index>
        <div class="d-flex justify-content-between align-items-start gap-3 flex-wrap mb-4">
            <div>
                <h2 class="mb-1">{{ t('sidebar.trainer_commissions') }}</h2>
                ...
                <table class="table table-hover">
                    <thead>
                        <tr>
                            ...
                            <th>{{ t('staff_reports.initial_commission') }}</th>
                            <th>{{ t('staff_reports.total_reassigned') }}</th>
                            <th>{{ t('staff_reports.unearned_cancelled') }}</th>
                            ...
                            <th>{{ t('staff_reports.generation_stopped') }}</th>
