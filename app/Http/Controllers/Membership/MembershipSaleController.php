...
class MembershipSaleController extends Controller
{
    ...
    {
        $data = $this->membershipSaleService->formOptions((int) $person);
        $data['gateway'] = [
            'url' => config('hdm.gateway.url', 'http://localhost/hdm-gateway/index.php'),
            'token' => config('hdm.gateway.token', 'SECRET_PROXY_TOKEN'),
        ];

        // return Inertia::render('MembershipSales/Create', $this->membershipSaleService->formOptions((int) $person));

        return Inertia::render('MembershipSales/Create', $data);
